...
to setup
  ...
  set-default-shape bugs1 "bug"
  set-default-shape bugs2 "bug"
  set-default-shape bugs3 "bug"
  set-default-shape bugs4 "bug"
  set-default-shape bugs5 "bug"
  set-default-shape bugs6 "bug"
  set-default-shape predators "bird"
  ask patches [ set pcolor white ]   ;; white background
  
  create-bugs1 initial-bugs-each-speed [ set speed 1 ]
  create-bugs2 initial-bugs-each-speed [ set speed 2 ]
  create-bugs3 initial-bugs-each-speed [ set speed 3 ]
  create-bugs4 initial-bugs-each-speed [ set speed 4 ]
  create-bugs5 initial-bugs-each-speed [ set speed 5 ]
  create-bugs6 initial-bugs-each-speed [ set speed 6]
...
to go
  ...
  every 0.03 [
    ...
    ask bugs1 [ set-color move-bugs kill-overPopulation]
    ask bugs2 [ set-color move-bugs kill-overPopulation]
    ask bugs3 [ set-color move-bugs kill-overPopulation]
    ask bugs4 [ set-color move-bugs kill-overPopulation]
    ask bugs5 [ set-color move-bugs kill-overPopulation]
    ask bugs6 [ set-color move-bugs kill-overPopulation]
  ...
  every 1 [
    if count bugs1 > 0 [ask bugs1 [breed-on-collision1]]
    if count bugs2 > 0 [ask bugs2 [breed-on-collision2]]
    if count bugs3 > 0 [ask bugs3 [breed-on-collision3]]
    if count bugs4 > 0 [ask bugs4 [breed-on-collision4]]
    if count bugs5 > 0 [ask bugs5 [breed-on-collision5]]
    if count bugs6 > 0 [ask bugs6 [breed-on-collision6]]
    ]
end
...
to breed-on-collision1
  let breeders1 [bugs1 in-radius (0.1)] of one-of bugs1
  ;; no prey here? oh well
  ;if not any? breeders [ stop ]
  ;; eat only one of the bugs at the mouse location
  ask one-of breeders1 [
    if any? other turtles-here [ ask one-of bugs1 [ hatch 1 [ set speed 1 rt random 360 setxy random-xcor random-ycor ] ] ]
...
to breed-on-collision2
  
  let breeders2 [bugs2 in-radius (0.1)] of one-of bugs2
     ask one-of breeders2 [
        if any? other turtles-here [ ask one-of bugs2 [ hatch 1 [ set speed 2 rt random 360 setxy random-xcor random-ycor ] ] ]
     ]
end


to breed-on-collision3

let breeders3 [bugs3 in-radius (0.1)] of one-of bugs3
     ask one-of breeders3 [
        if any? other turtles-here [ ask one-of bugs3 [ hatch 1 [ set speed 3 rt random 360 setxy random-xcor random-ycor ] ] ]
     ]

end


to breed-on-collision4

let breeders4 [bugs4 in-radius (0.1)] of one-of bugs4
     ask one-of breeders4 [
        if any? other turtles-here [ ask one-of bugs4 [ hatch 1 [ set speed 4 rt random 360 setxy random-xcor random-ycor ] ] ]
     ]

end


to breed-on-collision5

let breeders5 [bugs5 in-radius (0.1)] of one-of bugs5
     ask one-of breeders5 [
        if any? other turtles-here [ ask one-of bugs5 [ hatch 1 [ set speed 5 rt random 360 setxy random-xcor random-ycor ] ] ]
     ]
...
to breed-on-collision6

let breeders6 [bugs6 in-radius (0.1)] of one-of bugs6
     ask one-of breeders6 [
        if any? other turtles-here [ ask one-of bugs6 [ hatch 1 [ set speed 6 rt random 360 setxy random-xcor random-ycor ] ] ]
     ]

end


to kill-overPopulation
  if count turtles > 400[
    
    if count bugs1 > 0 [ask one-of bugs1 [die]]
    if count bugs2 > 0 [ask one-of bugs2 [die]]
    if count bugs3 > 0 [ask one-of bugs3 [die]]
    if count bugs4 > 0 [ask one-of bugs4 [die]]
    if count bugs5 > 0 [ask one-of bugs5 [die]]
    if count bugs6 > 0 [ask one-of bugs6 [die]]
    
    ;;ask one-of bugs1 [die] ask one-of bugs2 [die] ask one-of bugs3 [die] ask one-of bugs4 [die] ask one-of bugs5 [die] ask one-of bugs6 [die]
...
to set-color  ;; turtle procedure
  ifelse show-colors? 
    [ set color item (speed - 1) [violet blue green yellow orange red] ]
    [ set color gray]
end
...
true
false
"" ";; the HISTOGRAM primitive can't make a multi-colored histogram,\n;; so instead we plot each bar individually\nclear-plot\n\n  set-current-plot-pen word \"pen\" 6\n  plotxy 6 count bugs6\n  \n  set-current-plot-pen word \"pen\" 4\n  plotxy 4 count bugs4\n\n  set-current-plot-pen word \"pen\" 5\n  plotxy 5 count bugs5\n  \n  set-current-plot-pen word \"pen\" 1\n  plotxy 1 count bugs1\n  \n  set-current-plot-pen word \"pen\" 2\n  plotxy 2 count bugs2\n  \n  set-current-plot-pen word \"pen\" 3\n  plotxy 3 count bugs3"
PENS
...
140
bugs
(count bugs1 + count bugs2 + count bugs3 + count bugs4 + count bugs5 + count bugs6)
0
1
11
...
<experiments>
  <experiment name="experiment" repetitions="20" runMetricsEveryStep="false">
    ...
    <metric>count bugs with [speed = 1]</metric>
    <metric>count bugs with [speed = 2]</metric>
    <metric>count bugs with [speed = 3]</metric>
    <metric>count bugs with [speed = 4]</metric>
    <metric>count bugs with [speed = 5]</metric>
    <metric>count bugs with [speed = 6]</metric>
    <enumeratedValueSet variable="initial-bugs-each-speed">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wiggle?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="flee?">
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment2" repetitions="20" runMetricsEveryStep="false">
    ...
    <go>go</go>
    <timeLimit steps="100"/>
    <metric>count bugs1</metric>
    <metric>count bugs2</metric>
    <metric>count bugs3</metric>
    <metric>count bugs4</metric>
    <metric>count bugs5</metric>
    <metric>count bugs6</metric>
